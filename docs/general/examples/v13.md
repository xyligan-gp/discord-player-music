...
```JS
if(command === '!play') {
    if(!message.member.voice.channel) return message.channel.send({ content: 'Join to the voice channel!' });

    const query = args.join(' ');
    if(!query) return message.channel.send({ content: 'Enter your search request!' });

    player.searchSong(message.member, query, message.channel).then(data => {
        if(!data[0].index) return;

        return message.channel.send({ content: data.map(song => `**[${song.index}]** - **${song.title}** [${song.duration.hours}:${song.duration.minutes}:${song.duration.seconds}]`).join('\n').toString() });
    }).catch(error => {
...
```JS
if(command === '!join') {
    if(!message.member.voice.channel) return message.channel.send({ 'Join to the voice channel!' });

    client.player.voice.join(message.member).then(data => {
        return message.channel.send({ content: `Bot joined the **${data.voiceChannel.name}** channel!` });
    }).catch(error => {
        return message.channel.send({ content: error.message });
    })
}
...
if(command === '!leave') {
    ...
    client.player.join.leave(message.member).then(data => {
        return message.channel.send({ content: `Bot left the voice channel **${data.voiceChannel.name}**` });
    }).catch(error => {
        return message.channel.send({ content: error.message });
    })
}
...
if(command === '!skip') {
    ...
    client.player.skip(message.guild).then(data => {
        return message.channel.send({ content: `Song successfully skiped!`} );
    }).catch(error => {
        return message.channel.send({ content: error.message });
    })
}
...
if(command === '!stop') {
    ...
    client.player.stop(message.guild).then(status => {
        return message.channel.send({ content: 'Playing server queue stopped!' });
    }).catch(error => {
        return message.channel.send({ content: error.message });
    })
}
