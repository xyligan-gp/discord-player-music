...
export class DatabaseManager {
    ...
    constructor(player: Player) {
        if(!player) throw new PlayerError(errors.default.requireParam.replace('{param}', 'player').replace('{data}', '<DatabaseManager>'));
    ...
    public write(data?: object): void {
        writeFileSync(this.config.path, JSON.stringify(data ? data : this.storage, null, '\t'));
    }
    ...
    public initGuild(guildID: string): void {
        if(!this.storage[guildID]) this.storage[guildID] = {
            playlists:    []
        };
    ...
    private init(): void {
        setInterval(async () => {
            if(!existsSync(this.config.path)) writeFileSync(this.config.path, '{}', 'utf-8');
