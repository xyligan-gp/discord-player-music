...
export class FiltersManager {
    ...
    constructor(player: Player) {
        if(!player) throw new PlayerError(errors.default.requireParam.replace('{param}', 'player').replace('{data}', '<FiltersManager>'));
    ...
    public add(key: string, value: string): Promise<DefaultData | ErrorData> {
        return new Promise(async (res, rej) => {
            if(!key) throw new PlayerError(this.player.errors.default.requireParam.replace('{param}', 'key').replace('{data}', '<FilterManager>.add'));
            if(!value) throw new PlayerError(this.player.errors.default.requireParam.replace('{param}', 'value').replace('{data}', '<FilterManager>.add'));
            
            if(this.storage.get(key)) return res({ error: { code: 500, message: this.player.errors.filters.exists.replace('{name}', key) } });
    ...
    public isExists(key: string): Promise<boolean> {
        return new Promise(async (res, rej) => {
            if(!key) throw new PlayerError(this.player.errors.default.requireParam.replace('{param}', 'key').replace('{data}', '<FilterManager>.isExists'));
    ...
    public get(key: string): Promise<PlayerFilter | ErrorData> {
        return new Promise(async (res, rej) => {
            if(!key) throw new PlayerError(this.player.errors.default.requireParam.replace('{param}', 'key').replace('{data}', '<FilterManager>.get'));

            const filterValue = this.storage.get(key);
            if(!filterValue) return res({ error: { code: 404, message: this.player.errors.filters.notFound.replace('{name}', key) } });
    ...
    public delete(key: string): Promise<DefaultData | ErrorData> {
        return new Promise(async (res, rej) => {
            if(!key) throw new PlayerError(this.player.errors.default.requireParam.replace('{param}', 'key').replace('{data}', '<FilterManager>.delete'));

            if(!this.storage.get(key)) return res({ error: { code: 404, message: this.player.errors.filters.notFound.replace('{name}', key) } });
