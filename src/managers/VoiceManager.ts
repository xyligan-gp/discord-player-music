    ...
    return new Promise(async (res, rej) => {
      if (!channel) {
        throw new PlayerError(
          this.player.errors.default.requireParam
            ...
            .replace("{data}", "<VoiceManager>.join")
        );
      }

      const guildClientMember = channel.guild.members.cache.get(
        this.player.client.user.id
      );
      if (guildClientMember?.voice?.channel) {
        return res({
          error: {
            code: 404,
            message: this.player.errors.voice.alreadyConnected
              .replace("{id}", this.player.client.user.id)
              .replace("{channelID}", guildClientMember.voice.channel.id),
          },
        });
      }
      ...
      if (connection) {
        ...
        return res({
          error: {
            code: 503,
            message: this.player.errors.voice.failConnect.replace(
              "{id}",
              guildClientMember.voice.channel.id
            ),
          },
        });
      }
    ...
    return new Promise(async (res, rej) => {
      if (!guildID) {
        throw new PlayerError(
          this.player.errors.default.requireParam
            ...
            .replace("{data}", "<VoiceManager>.leave")
        );
      }
      ...
      if (!guild) {
        return res({
          error: {
            code: 404,
            message: this.player.errors.other.unknownGuild
              .replace("{id}", guildID)
              .replace("{clientID}", this.player.client.user.id),
          },
        });
      }

      const guildClientMember = guild.members.cache.get(
        this.player.client.user.id
      );
      if (!guildClientMember?.voice?.channel) {
        return res({
          error: {
            code: 404,
            message: this.player.errors.voice.notConnected.replace(
              "{id}",
              this.player.client.user.id
            ),
          },
