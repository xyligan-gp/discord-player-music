...
export class PlaylistsManager {
  ...
  public create(
    ...
    return new Promise(async (res, rej) => {
      ...
      const guildPlaylists: GuildPlaylist[] = storage[guildID]["playlists"];
  ...
  ): Promise<GuildPlaylist | ErrorData> {
    return new Promise(async (res, rej) => {
      if (!guildID) {
        throw new PlayerError(
          this.player.errors.default.requireParam
            .replace("{param}", "guildID")
            .replace("{data}", "<PlaylistsManager>.addTrack")
        );
      }
      ...
      if (guildPlaylists.length >= 1) {
        if (data.id && data.id.length > 0) {
          const guildPlaylist = guildPlaylists.find(
            (playlist) => playlist.id === data.id
          );

          if (guildPlaylist) {
            const playlistTrack = guildPlaylist.tracks.find(
              (track) => track.url === data.track.url
            );

            if (!playlistTrack) {
              guildPlaylist.updated = Date.now();
              guildPlaylist.duration +=
                await this.player.utils.getTrackDuration(data.track.url);
              ...
              return res({
                error: {
                  code: 404,
                  message: this.player.errors.playlists.existsTrack
                    .replace("{url}", data.track.url)
                    .replace("{id}", data.id),
                },
              });
            }
            ...
            return res({
              error: {
                code: 404,
                message: this.player.errors.playlists.notFound
                  .replace("{id}", data.id)
          ...
          this.create(guildID, {
            author: this.player.client.user.id,
            track: data.track,
          });
        }
      } else {
    ...
    return new Promise(async (res, rej) => {
      if (!guildID) {
        throw new PlayerError(
          this.player.errors.default.requireParam
            .replace("{param}", "guildID")
            .replace("{data}", "<PlaylistsManager>.play")
        );
      }
      ...
      if (guildPlaylists.length <= 0) {
        return res({
          error: {
            code: 404,
            message: this.player.errors.playlists.guildNotFound.replace(
              "{id}",
              guildID
            ),
          },
    ...
    return new Promise(async (res, rej) => {
      if (!guildID) {
        throw new PlayerError(
          this.player.errors.default.requireParam
            .replace("{param}", "guildID")
            .replace("{data}", "<PlaylistsManager>.removeTrack")
        );
      }
      ...
      if (!playlistTracks[i]) {
        return res({
          error: {
            code: 404,
            message: this.player.errors.playlists.trackNotFound
      ...
      guildPlaylist.duration =
        guildPlaylist.duration -
        (await this.player.utils.getTrackDuration(playlistTracks[i]?.url));
