...
import { Events, GuildQueueState, Loop } from "../PlayerEnums";
...
export class QueueManager {
    ...
    return new Promise(async (res, rej) => {
      ...
      if (!guildConnection) {
        guildConnection = joinVoiceChannel({
          guildId: guild.id,
          channelId: track.channel.voice.id,
      ...
      if (!guildQueue) {
        ...
        return res(
          this.storage
            .get(guild.id)
            ?.tracks.find((guildTrack) => guildTrack.url === track.url) != null
        );
      }
    });
  }
  ...
  ): Promise<DefaultData | ErrorData> {
    return new Promise(async (res, rej) => {
      if (!guildID) {
        throw new PlayerError(
          this.player.errors.default.requireParam
            .replace("{param}", "guildID")
            .replace("{data}", "<QueueManager>.setState")
        );
      }
      ...
      if (!guildQueue) {
        return res({
          error: {
            code: 404,
            message: this.player.errors.default.missingQueue.replace(
              "{id}",
              guildID
            ),
          },
      ...
      if (
        type &&
        [GuildQueueState.PAUSED, GuildQueueState.PLAYING].includes(type)
      ) {
        const newState = type;

        if (guildQueue.state.includes(newState))
          return res({
            error: {
              code: 404,
              message: this.player.errors.other.unknownState
                .replace("{state}", newState)
                .replace("{id}", guildID),
            },
          });

        if (newState.includes(GuildQueueState.PAUSED)) {
    ...
    return new Promise(async (res, rej) => {
      if (!guildID) {
        throw new PlayerError(
          this.player.errors.default.requireParam
            .replace("{param}", "guildID")
            .replace("{data}", "<QueueManager>.progress")
        );
      }
      ...
      const emptyProgress = this.player.utils.createEmptyProgress();

      if (!guildQueue.dispatcher) {
        return res({ bar: emptyProgress, percents: "0%" });
      }
      ...
      const trackDuration = Math.floor(
        Number(track.duration.hours) * 3600 +
          Number(track.duration.minutes) * 60 +
          Number(track.duration.seconds)
      );
      const totalSeconds = Math.floor(trackDuration * 1000);
      const alreadyPlayedSeconds = Math.floor(
        (guildQueue.dispatcher.state as any).resource.playbackDuration || 0
      );

      if (isNaN(this.player.options.progressConfig.size))
        return res({ bar: emptyProgress, percents: "0%" });
      ...
      if (!alreadyPlayedSeconds)
        return res({ bar: emptyProgress, percents: "0%" });
      if (isNaN(alreadyPlayedSeconds)) {
        return res({ bar: emptyProgress, percents: "0%" });
      }
      ...
      if (alreadyPlayedSeconds < totalSeconds) {
        ...
        const progress = Math.round(
          this.player.options.progressConfig.size * percentage
        );

        const empty = this.player.options.progressConfig.size - progress;
        const progressBar = this.player.options.progressConfig.line
          .repeat(progress)
          .replace(/.$/, this.player.options.progressConfig.slider);

        const emptyProgressBar =
          this.player.options.progressConfig.line.repeat(empty);
        const percents = Math.floor(percentage * 100);

        if (percents < 10) {
          return res({
            bar: this.player.utils.createEmptyProgress(),
            percents: `${percents}%`,
          });
        } else {
          return res({
            bar: progressBar + emptyProgressBar,
            percents: `${percents}%`,
          });
        }
      } else {
        return res({
          bar: this.player.options.progressConfig.line.repeat(
            this.player.options.progressConfig.size + 2
          ),
          percents: `${(alreadyPlayedSeconds / totalSeconds) * 100}%`,
        });
      }
    });
  }
    ...
    return new Promise(async (res, rej) => {
      if (!guildID) {
        throw new PlayerError(
          this.player.errors.default.requireParam
            .replace("{param}", "guildID")
            .replace("{data}", "<QueueManager>.dispatcherInfo")
        );
      }
      ...
      return res({
        loop: { track: guildQueue.loop.track, queue: guildQueue.loop.queue },
        filter: guildQueue.filter,
        state: guildQueue.state,
    ...
    return new Promise(async (res, rej) => {
      if (!guildID) {
        throw new PlayerError(
          this.player.errors.default.requireParam
            .replace("{param}", "guildID")
            .replace("{data}", "<QueueManager>.trackInfo")
        );
      }
      ...
      return res(
        guildTracks.indexOf(guildTracks[index]) == -1
          ? guildTracks[0]
          : guildTracks[index]
      );
    });
  ...
  public setFilter(
    guildID: string,
    filter?: Filter
  ): Promise<PlayerFilter | ErrorData> {
    return new Promise(async (res, rej) => {
      if (!guildID) {
        throw new PlayerError(
          this.player.errors.default.requireParam
            .replace("{param}", "guildID")
            .replace("{data}", "<QueueManager>.setFilter")
        );
      }
      ...
      if ("error" in playerFilter) {
        return res({
          error: {
            code: 404,
            message: this.player.errors.filters.notFound.replace(
              "{name}",
              filter || "clear"
            ),
          },
      ...
      guildQueue.filter = playerFilter.name.includes("clear")
        ? { name: null, value: null }
        : playerFilter;

      this.player.initGuildTrack(guildID, guildQueue.tracks[0]);
   ...
   *
   * @returns {Promise<LoopModes | ErrorData>} Loop setup status or object in error
   */
  public setLoop(guildID: string, type?: Loop): Promise<LoopModes | ErrorData> {
    return new Promise(async (res, rej) => {
      if (!guildID) {
        throw new PlayerError(
          this.player.errors.default.requireParam
            .replace("{param}", "guildID")
            .replace("{data}", "<QueueManager>.setLoop")
        );
      }
      ...
      if (type) {
        switch (type) {
          case Loop.QUEUE: {
            ...
            return res({
              track: guildQueue.loop.track,
              queue: guildQueue.loop.queue,
            });
          }
    ...
    return new Promise(async (res, rej) => {
      if (!guildID) {
        throw new PlayerError(
          this.player.errors.default.requireParam
            .replace("{param}", "guildID")
            .replace("{data}", "<QueueManager>.setVolume")
        );
      }

      if (value && typeof value != "number") {
        throw new PlayerError(
          this.player.errors.default.invalidType
            .replace("{value}", "value")
            .replace("{data}", "number")
        );
      }
      ...
      if (value && isNaN(value)) {
        throw new PlayerError(
          this.player.errors.default.invalidValue
            .replace("{value}", "value")
            .replace("{data}", this.player.options.defaultVolume.toString())
        );
      }
      ...
      (guildQueue.dispatcher.state as any).resource?.volume.setVolume(
        value / this.player.options.defaultVolume
      );
    ...
    return new Promise(async (res, rej) => {
      if (!guildID) {
        throw new PlayerError(
          this.player.errors.default.requireParam
            .replace("{param}", "guildID")
            .replace("{data}", "<QueueManager>.skipTrack")
        );
      }
      ...
      if (guildQueue.tracks.length === 1) {
        if (
          guildQueue.connection.state.status != VoiceConnectionStatus.Destroyed
        ) {
      ...
      return res({
        current: guildQueue.tracks[0] || null,
        next: guildQueue.tracks[1] || null,
      });
    });
  }
  ...
  public removeTrack(
    guildID: string,
    index?: number
  ): Promise<RemoveTrackData | ErrorData> {
    return new Promise(async (res, rej) => {
      if (!guildID) {
        throw new PlayerError(
          this.player.errors.default.requireParam
            .replace("{param}", "guildID")
            .replace("{data}", "<QueueManager>.removeTrack")
        );
      }
      ...
      const guildTracks = guildQueue.tracks;
      if (guildTracks.indexOf(guildTracks[index]) == -1) {
        return res({
          error: {
            code: 404,
            message: this.player.errors.other.unknownGuildTrack
    ...
    return new Promise(async (res, rej) => {
      if (!guildID) {
        throw new PlayerError(
          this.player.errors.default.requireParam
            .replace("{param}", "guildID")
            .replace("{data}", "<QueueManager>.seek")
        );
      }
      ...
      const stream = await ytdl(guildQueue.tracks[0].url, {
        seek: value,
      ...
      (guildQueue.dispatcher.state as any).resource.playbackDuration =
        Math.floor(value * 1000);
      (guildQueue.dispatcher.state as any).resource.volume.setVolume(
        guildQueue.volume / this.player.options.defaultVolume
      );
   ...
   *
   * @returns {Promise<PlayerTrack[] | ErrorData>} Shuffled list of tracks in the queue
   */
  public shuffle(guildID: string): Promise<PlayerTrack[] | ErrorData> {
    return new Promise(async (res, rej) => {
      if (!guildID) {
        throw new PlayerError(
          this.player.errors.default.requireParam
            .replace("{param}", "guildID")
            .replace("{data}", "<QueueManager>.shuffle")
        );
      }
      ...
      if (!guildQueue) {
        return res({
          error: {
            ...
            message: this.player.errors.default.missingQueue.replace(
              "{id}",
              this.player.client.user.id
            ),
          },
        });
      }

      if (guildQueue.tracks.length <= 2) {
        return res({
          error: { code: 403, message: this.player.errors.other.shuffleInfo },
        });
      }
      ...
      for (let i = guildQueue.tracks.length - 1; i > 0; i--) {
        ...
        [guildQueue.tracks[i], guildQueue.tracks[index]] = [
          guildQueue.tracks[index],
          guildQueue.tracks[i],
        ];
      }
    ...
    return new Promise(async (res, rej) => {
      if (!guildID) {
        throw new PlayerError(
          this.player.errors.default.requireParam
            .replace("{param}", "guildID")
            .replace("{data}", "<QueueManager>.stop")
        );
      }
    ...
    return new Promise(async (res, rej) => {
      if (!guildID) {
        throw new PlayerError(
          this.player.errors.default.requireParam
            .replace("{param}", "guildID")
            .replace("{data}", "<QueueManager>.clear")
        );
      }

      if (!this.storage.get(guildID)) {
        return res({
          error: {
            code: 404,
            message: this.player.errors.default.missingQueue.replace(
