...
import { ProgressType } from '../PlayerEnums';
...
export class UtilsManager {
     ...
     * 
     * @example
     * const progressBar = client.player.utils.createProgress();
     * 
     * // Result: '🔘▬▬▬▬▬▬▬▬▬▬'
     * console.log(progressBar);
     * 
     * @returns {string} Created progress bar
     */
    public createProgress(type?: ProgressType): string {
        if(!type) type = ProgressType.EMPTY;

        switch(type) {
            case ProgressType.EMPTY: {
            ...
            case ProgressType.FULL: {
                let bar: string = '';

                for(let i = 0; i < this.player.options.progressConfig.size - 1; i++) {
                    if((i + 1) >= this.player.options.progressConfig.size) bar += this.player.options.progressConfig.slider;
                    else bar += this.player.options.progressConfig.line;
                }
