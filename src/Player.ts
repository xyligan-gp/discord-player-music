...
import { Client, Guild, GuildMember, Message, MessageReaction, TextChannel, User, VoiceChannel } from 'discord.js';
import { AudioPlayer, AudioPlayerStatus, createAudioResource, entersState, StreamType, VoiceConnection, VoiceConnectionStatus } from '@discordjs/voice';
...
import { Collector, Events, GuildQueueState, Search } from './PlayerEnums';
import { PlayerOptions } from '../types/PlayerOptions';
import { ErrorData, LyricsData, PlayerTrack, PlayerError as PlayerErrorData, PlayerQueue, CollectorData, PlayerFilter, PlaylistTrack } from '../types/PlayerData';
...
export class Player extends PlayerEmitter {
    ...
    constructor(client: Client, options?: PlayerOptions) {
        ...
        if(!client) throw new PlayerError(errors.default.requireParam.replace('{param}', 'client').replace('{data}', '<Player>'));
        ...
        this.on(Events.ERROR, async (data: PlayerErrorData) => {
            if(data.channel.text) {
                if(data.error.message.includes('Status code: 403')) {
                    this.queue.get(data.channel.text.guild.id).then(async (guildQueue: PlayerQueue) => {
                        if(guildQueue?.channel) return this.initGuildTrack(guildQueue.channel.text.guild.id, guildQueue.tracks[0]);
                    })
                }
            }else{
                this.queue.get(data.channel.text.guild.id).then(async (guildQueue: PlayerQueue) => {
     ...
     */
    public initGuildTrack(guildID: string, track: PlayerTrack): Promise<void | ErrorData> {
        return new Promise(async (res, rej) => {
            if(!guildID) throw new PlayerError(this.errors.default.requireParam.replace('{param}', 'guildID').replace('{data}', '<Player>.initGuildTrack'));

            const guildQueue = await this.queue.get(guildID);
            if('error' in guildQueue) return res({ error: { code: 404, message: this.errors.default.missingQueue.replace('{id}', guildID) } });
            ...
            try {
                const stream = await ytdl(track.url, { filter: guildQueue.filter.value });
                ...
                try {
                    await entersState(guildConnection, VoiceConnectionStatus.Ready, 5_000);

                    guildDispatcher.play(resource);
                    guildDispatcher.setMaxListeners(0);

                    (guildConnection.state as any).subscription?.player ? false : guildConnection.subscribe(guildDispatcher);
                }catch(error){
                    if(guildQueue) this.queue.delete(guildID);
                    if(guildConnection.state.status != VoiceConnectionStatus.Destroyed) guildConnection.destroy();

                    return this.emit(Events.ERROR, { channel: { text: track.channel.text, voice: track.channel.voice }, requested: track.requested, method: 'initGuildTrack', error: error });
                }
                ...
                guildDispatcher.once(AudioPlayerStatus.Idle, async () => {
                    ...
                    if(guildQueue.loop.track) this.initGuildTrack(guildID, guildQueue.tracks[0]);
                ...
                guildDispatcher.on('error', async error => {
                    ...
                    this.emit(Events.ERROR, { channel: { text: track.channel.text, voice: track.channel.voice }, requested: track.requested, method: 'initGuildTrack', error: error });
                });

                (guildDispatcher.state as any).resource.volume.setVolume(guildQueue.volume / this.options.defaultVolume);
     ...
     * 
     * @returns {Promise<Array<PlayerTrack> | ErrorData>} Array with search results or error object
     */
    public search(query: string, member: GuildMember, channel: TextChannel, isPlaylist?: boolean): Promise<Array<PlayerTrack> | ErrorData> {
        return new Promise(async (res, rej) => {
            if(!query) throw new PlayerError(this.errors.default.requireParam.replace('{param}', 'query').replace('{data}', '<Player>.search'));
            if(!member) throw new PlayerError(this.errors.default.requireParam.replace('{param}', 'member').replace('{data}', '<Player>.search'));
            if(!channel) throw new PlayerError(this.errors.default.requireParam.replace('{param}', 'channel').replace('{data}', '<Player>.search'));

            const voiceChannel = member?.voice?.channel as VoiceChannel;
            if(!voiceChannel) return res({ error: { code: 404, message: this.errors.voice.notConnected.replace('{id}', member.user.id) } });

            if(query.startsWith('http://') || query.startsWith('https://')) {
                const trackInfo = await ytdl.getInfo(query).catch((error: Error) => {
                    return res({ error: { code: 404, message: this.errors.other.unknownTrack.replace('{data}', query) } });
                })

                if(!trackInfo) return res({ error: { code: 404, message: this.errors.other.unknownTrack.replace('{data}', query) } });

                const duration = this.utils.formatnumbers([Math.floor(Number(trackInfo.videoDetails.lengthSeconds) / 3600), Math.floor(Number(trackInfo.videoDetails.lengthSeconds) / 60 % 60), Math.floor(Number(trackInfo.videoDetails.lengthSeconds) % 60)]);
                ...
                const results: Array<PlayerTrack> = [];

                const searchResults = await searchTrack(query);
                if(searchResults.videos.length <= 0) return res({ error: { code: 404, message: this.errors.other.unknownTrack.replace('{data}', query) } });

                for(let i = 0; searchResults.videos.length && searchResults.videos.length < this.options.searchResultsLimit ? i < searchResults.videos.length : i < this.options.searchResultsLimit; i++) {
                    const video = searchResults.videos[i];
                    const duration = this.utils.formatnumbers([Math.floor(searchResults.videos[i].duration.seconds / 3600), Math.floor(searchResults.videos[i].duration.seconds / 60 % 60), Math.floor(searchResults.videos[i].duration.seconds % 60)]);
    ...
    public lyrics(query: string): Promise<LyricsData | ErrorData> {
        return new Promise(async (res, rej) => {
            if(!query) throw new PlayerError(this.errors.default.requireParam.replace('{param}', 'query').replace('{data}', '<Player>.getLyrics'));

            const result: string = await searchLyrics(query);
            if(!result) return res({ error: { code: 404, message: this.errors.other.notLyrics.replace('{query}', query) } });
     ...
     * @param {Array<PlayerTrack>} results Array with search results
     * 
     * @example
     * const searchData = await client.player.search('Xtrullor - Samsara', message.member, message.channel);
    ...
    public initQueueTrack(index: number, results: Array<PlayerTrack>): Promise<void> {
        return new Promise(async (res, rej) => {
            if(!index) throw new PlayerError(this.errors.default.requireParam.replace('{param}', 'index').replace('{data}', '<Player>.initQueueTrack'));
            if(!results) throw new PlayerError(this.errors.default.requireParam.replace('{param}', 'results').replace('{data}', '<Player>.initQueueTrack'));
     ...
     * @param {Array<PlayerTrack>} results Search results
    ...
    public createCollector(type: Collector, message: Message, results: Array<PlayerTrack>, userID?: string): Promise<CollectorData | ErrorData> {
        return new Promise(async (res, rej) => {
            if(!type) throw new PlayerError(this.errors.default.requireParam.replace('{param}', 'type').replace('{data}', '<Player>.createCollector'));

            switch(type) {
                case Collector.MESSAGE: {
                    if(!message) throw new PlayerError(this.errors.default.requireParam.replace('{param}', 'message').replace('{data}', '<Player>.createReactionsCollector'));
                    if(!results) throw new PlayerError(this.errors.default.requireParam.replace('{param}', 'results').replace('{data}', '<Player>.createReactionsCollector'));

                    if(results.length <= 0) throw new PlayerError(this.errors.collectors.unknownResults.replace('{type}', 'Message'));

                    const filter = (msg: Message) => msg.author.id.includes(userID || message.author.id);
                    const collector = message.channel.createMessageCollector({ filter, max: this.options.collectorsConfig.message.attempts, time: ms(this.options.collectorsConfig.message.time) as number });
                    ...
                    collector.on('collect', async collectMsg => {
                        ...
                        if(!isNaN(collectValue)) {
                            if(collectValue <= 0) {
                                ...
                                return res({ error: { code: 404, message: this.errors.collectors.smallValue.replace('{type}', 'Message').replace('{value}', collectValue.toString()).replace('{min}', '1') } });
                            }

                            if(collectValue > results.length) {
                                collector.stop();

                                return res({ error: { code: 404, message: this.errors.collectors.largeValue.replace('{type}', 'Message').replace('{value}', collectValue.toString()).replace('{max}', results.length.toString()) } });
                            }
                ...
                case Collector.REACTION: {
                    if(!message) throw new PlayerError(this.errors.default.requireParam.replace('{param}', 'message').replace('{data}', '<Player>.createReactionsCollector'));
                    if(!results) throw new PlayerError(this.errors.default.requireParam.replace('{param}', 'results').replace('{data}', '<Player>.createReactionsCollector'));
                    if(!userID) throw new PlayerError(this.errors.default.requireParam.replace('{param}', 'userID').replace('{data}', '<Player>.createReactionsCollector'));

                    if(results.length <= 0) throw new PlayerError(this.errors.collectors.unknownResults.replace('{type}', 'Message'));
                    if(results.length > 10) throw new PlayerError(this.errors.collectors.unknownCollector.replace('{type}', 'Reaction').replace('{value}', '10').replace('{collector}', 'Message'));
                    ...
                    const filter = (reaction: MessageReaction, user: User) => reaction.message.id === message.id && user.id === userID;
                    const collector = message.createReactionCollector({ filter, time: ms(this.options.collectorsConfig.reaction.time) as number, maxEmojis: 1, max: this.options.collectorsConfig.reaction.attempts });
                    ...
                    collector.on('collect', async (r, u) => {
                        ...
                        reacts.forEach(react => {
                            if(emoji === react) {
                                ...
                                if(this.options.autoAddingTracks) this.initQueueTrack(index, results);
 ...
 * @prop {object} [databaseConfig] Database configuration object
 * @prop {string} [path=./dpm.playlists.json] Default path to module database file
 * @prop {string} [checkInterval=5s] Default interval for checking and updating the module database
 * @prop {object} [progressConfig] Progress bar configuration object
 * @prop {number} [size=11] Progress bar size
 * @prop {string} [line=▬] Progress bar line
 * @prop {string} [slider=🔘] Progress bar slider
 * @prop {object} [collectorsConfig] Collectors configuration object
 * @prop {object} [collectorsConfig.message] Message collector configuration object
 * @prop {string} [collectorsConfig.message.time=1m] Message collector run time
 * @prop {number} [collectorsConfig.message.attempts=1] Maximum number of attempts to select a track
 * @prop {object} [collectorsConfig.reaction] Reaction collector configuration object
 * @prop {string} [collectorsConfig.reaction.time=30s] Reaction collector ran time
 * @prop {number} [collectorsConfig.reaction.attempts=1] Maximum number of attempts to select a track
 * @prop {Array<string>} [collectorsConfig.reaction.reactions=[]] Array react for collector
 */
 ...
 * @event Player#error
 * 
 * @param {object} [channel] Error Channels
 * @param {TextChannel} [channel.text] Discord Guild text Channel
 * @param {VoiceChannel} [channel.voice] Discord Guild Voice Channel
 ...
 * @param {object} author Track author object
 * @param {string} author.name Track author name
 * @param {string} author.url Track author channel URL
 * @param {object} channel Track active channels
 * @param {TextChannel} channel.text Discord Guild Text Channel
 * @param {VoiceChannel} channel.voice Discord Guild Voice Channel
 * @param {Guild} guild Discord Guild
 * @param {User} requested Requested track user object
 * @param {object} duration Track duration
 * @param {string} duration.hours Track duration in hours
 * @param {string} duration.minutes Track duration in minutes
 * @param {string} duration.seconds Track duration in seconds
 */
 ...
 * @param {Array<PlaylistTrack>} tracks Playlist tracks array
 */
 ...
 * 
 * @param {object} channel Queue active channels
 * @param {TextChannel} channel.text Discord Guild Text Channel
 * @param {VoiceChannel} channel.voice Discord Guild Voice Channel
 * @param {AudioPlayer} dispatcher Guild Audio Dispatcher
 * @param {Array<PlayerTrack>} tracks Guild Tracks Array
 * @param {VoiceConnection} connection Discord Guild Client Voice Connection
 * @param {object} loop Loop object
 * @param {boolean} loop.track Loop current track status
 * @param {boolean} loop.queue Loop guild queue status
 ...
 * @prop {object} author Track author object
 * @prop {string} author.name Track author name
 * @prop {string} author.url Track author channel URL
 * @prop {object} channel Track active channels
 * @prop {TextChannel} channel.text Discord Guild Text Channel
 * @prop {VoiceChannel} channel.voice Discord Guild Voice Channel
 * @prop {Guild} guild Discord Guild
 * @prop {User} requested Requested track user object
 * @prop {object} duration Track duration
 * @prop {string} duration.hours Track duration in hours
 * @prop {string} duration.minutes Track duration in minutes
 * @prop {string} duration.seconds Track duration in seconds
 */

/**
 * @typedef {object} PlayerQueue
 * 
 * @prop {object} channel Queue active channels
 * @prop {TextChannel} channel.text Discord Guild Text Channel
 * @prop {VoiceChannel} channel.voice Discord Guild Voice Channel
 * @prop {AudioPlayer} dispatcher Guild Audio Dispatcher
 * @prop {Array<PlayerTrack>} tracks Guild Tracks Array
 ...
 * @prop {Array<PlaylistTrack>} tracks Playlist tracks array
 */
 ...
 * @prop {Array<PlayerTrack>} data List of available tracks
 */
 ...
 * @prop {object} streamTime Stream duration object
 * @prop {string | number} streamTime.days Stream duration in days
 * @prop {string | number} streamTime.hours Stream duration in hours
 * @prop {string | number} streamTime.minutes Stream duration in minutes
 * @prop {string | number} streamTime.seconds Stream duration in seconds
 */
 ...
 * @prop {Array<PlayerTrack>} tracks Updated tracks list
 */

/**
 * @typedef {object} PlayerError
 * 
 * @prop {object} [channel] Error Channels
 * @prop {TextChannel} [channel.text] Discord Guild text Channel
 * @prop {VoiceChannel} [channel.voice] Discord Guild Voice Channel
